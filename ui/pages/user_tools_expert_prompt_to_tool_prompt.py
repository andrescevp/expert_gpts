...
from expert_gpts import ExpertAgentToolPrompt
...
dash.register_page(__name__, path_template="/user_tools/expert_prompt_to_tool_prompt")
COMPONENT_ID = "expert_prompt_to_tool_prompt"
...
def get_current_tools_prompts():
    with get_db_session() as session:
        # load dataframe from database with session
        return pd.read_sql_table(ExpertAgentToolPrompt.__tablename__, session.bind)
...
def layout():
    ...
    extra_children = []
    title = "LangChain Tools Engineer"
    description = """
    This tool allows you to create a prompts from a specific chatbot to be used as LangChain Tool Prompt.
    """
    # load all ExpertAgentToolPrompt models
    current_tools_prompts = get_current_tools_prompts()
    table = dash_table.DataTable(
        id="tools-prompt-table",
        data=current_tools_prompts.to_dict("records"),
        columns=[{"name": i, "id": i} for i in current_tools_prompts.columns],
        style_data={
            "whiteSpace": "normal",
            "height": "auto",
            "color": "black",
    ...
    extra_children.append(
        dbc.Card(
            [html.H4("Current tools prompts generated in the system"), table],
            className="p-2 mt-2",
        )
    )
...
def get_answer(data, chats_prevs):
    ...
    if data.answer:
        return dash.no_update
    answer = experts_manager.get_langchain_tools_expert_completion(
        data.current_user_prompt
    )
